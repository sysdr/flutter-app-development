...
final class DiscoveryCardShimmer extends StatelessWidget{
  ...
  @override Widget build(BuildContext context)=>Card(child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[
    ...
    Padding(padding:const EdgeInsets.all(AppSpacing.md),child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[
      ShimmerBox(animation:animation,width:double.infinity,height:18,borderRadius:AppSpacing.radiusSm),
      const SizedBox(height:AppSpacing.sm),
      FractionallySizedBox(widthFactor:0.75,child:ShimmerBox(animation:animation,width:double.infinity,height:13,borderRadius:AppSpacing.radiusSm)),
      const SizedBox(height:AppSpacing.sm),
      FractionallySizedBox(widthFactor:0.4,child:ShimmerBox(animation:animation,width:double.infinity,height:16,borderRadius:AppSpacing.radiusSm)),
    ])),
  ]));
}
