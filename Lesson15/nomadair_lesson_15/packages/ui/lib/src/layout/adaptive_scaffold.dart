...
final class AdaptiveScaffold extends StatefulWidget {
  ...
  @override
  State<AdaptiveScaffold> createState() => _S();
}
...
final class _S extends State<AdaptiveScaffold> {
  ...
  List<NavigationDestination> get _bar => widget.destinations
      .map(
  ...
  List<NavigationRailDestination> get _rail => widget.destinations
      .map(
  ...
  Widget build(BuildContext c) => LayoutBuilder(
        builder: (ctx, cs) {
          ...
          return KeyedSubtree(
            key: ValueKey<Object>('${style.runtimeType}_$_idx'),
            child: switch (style) {
              CompactNav() => _mob(body),
  ...
  Widget _mob(Widget body) => Scaffold(
        appBar: _bar2(),
        body: body,
        bottomNavigationBar: NavigationBar(
          selectedIndex: _idx,
          onDestinationSelected: _sel,
          destinations: _bar,
          height: 64,
        ),
      );
