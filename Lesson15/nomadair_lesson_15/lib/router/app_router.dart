...
abstract final class AppRouter {
  ...
  static final GoRouter router = GoRouter(
    ...
    redirect: (_, state) {
      if (state.matchedLocation.startsWith('/profile') && !isLoggedIn.value)
      ...
      StatefulShellRoute.indexedStack(
        ...
        branches: [
          StatefulShellBranch(routes: [GoRoute(
            ...
            routes: [GoRoute(path:'detail', builder:(_, state){
              final extra=state.extra;
              if(extra is DiscoveryDestination) return DestinationDetailScreen(destination:extra);
              final iata=state.uri.queryParameters['iata']??'';
              final dest=DiscoveryDestination.samples.where((d)=>d.iataCode==iata.toUpperCase()).firstOrNull;
              return dest==null?NotFoundScreen(location:'Destination $iata not found'):DestinationDetailScreen(destination:dest);
            })])]),
          StatefulShellBranch(routes: [GoRoute(
            ...
            routes: [GoRoute(path:'results',
              builder:(_, state)=>SearchResultsScreen(
                criteria:state.extra is SearchCriteria?state.extra as SearchCriteria:null))])]),
          StatefulShellBranch(routes: [GoRoute(
