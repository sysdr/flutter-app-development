...
/// Lesson 18 — SearchResultsScreen.
///
/// Change from L17: FlightResultCard.onTap now navigates to
///   FlightDetailScreen via context.push + GoRouter extra.
///
/// context.push (not context.go) so the user can go back to results.
...
final class _SearchResultsScreenState
      ...
      body: Column(children: [
        if (sub.isNotEmpty)
          Container(
            ...
            child: Text(sub, style: AppTypography.bodySmall.copyWith(
              color: t.brandPrimary))),
        Expanded(child: AnimatedSwitcher(
          duration: const Duration(milliseconds: 300),
          child: KeyedSubtree(
            key: ValueKey(notifier.state.runtimeType),
            child: switch (notifier.state) {
              FlightSearchIdle()    => _idle(t),
  ...
  Widget _idle(NomadThemeExtension t) => Center(
    child: Column(mainAxisSize: MainAxisSize.min, children: [
      ExcludeSemantics(child: Icon(Icons.flight_outlined, size: 64,
    ...
    return Column(children: [
      ...
      Expanded(child: ListView.separated(
        ...
        itemBuilder: (_, i) => FlightResultCard(
          ...
          flight: sorted[i],
          // L18: navigate to FlightDetailScreen with GoRouter extra
          onTap: () => context.push(
            NavigatorRoutes.flightDetail, extra: sorted[i])),
      )),
    ]);
  }

  Widget _error(String msg, criteria,
      FlightSearchNotifier n, NomadThemeExtension t) =>
