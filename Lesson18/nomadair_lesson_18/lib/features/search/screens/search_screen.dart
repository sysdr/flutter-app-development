...
/// Lesson 18 — SearchScreen.
///
/// Change from L17: adds a FAB that opens MasteryRubricScreen.
final class SearchScreen extends StatefulWidget {
  const SearchScreen({super.key,
    this.initialOrigin, this.initialDestination});
...
final class _SS extends State<SearchScreen>
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      // L18: FAB opens mastery rubric
      floatingActionButton: FloatingActionButton.small(
        tooltip: 'Mastery Rubric',
        backgroundColor: t.brandPrimary.withAlpha(220),
        onPressed: () =>
            context.push(NavigatorRoutes.masteryRubric),
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          DatePickerField(
            ...
            value: cr.departureDate, errorText: _dpe,
            onChanged: (d) {
              n.setDepartureDate(d);
          ...
          if (cr.isRoundTrip) ...[
            DatePickerField(
              label: 'Return Date',
              value: cr.returnDate,
              firstDate: cr.departureDate?.add(
                  const Duration(days: 1)),
              enabled: cr.departureDate != null,
              errorText: _re,
              onChanged: (d) {
                n.setReturnDate(d);
                setState(() => _re = null);
          ...
          PassengerCountSelector(
            value: cr.passengers,
            onChanged: n.setPassengers),
          const SizedBox(height: AppSpacing.md),
          CabinClassSelector(
            value: cr.cabinClass,
            onChanged: n.setCabinClass),
