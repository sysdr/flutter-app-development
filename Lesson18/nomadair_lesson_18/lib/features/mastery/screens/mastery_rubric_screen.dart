...
class _Assertion {
  const _Assertion({
    required this.tag, required this.label, this.checked = false});
  final String tag, label;
  final bool checked;
  _Assertion copyWith({bool? checked}) =>
      _Assertion(tag: tag, label: label,
          checked: checked ?? this.checked);
}

/// Lesson 18 — MasteryRubricScreen.
///
/// Interactive checklist of all 28 mastery assertions.
/// Students tap to mark assertions as verified during manual testing.
/// Run `python generate_lesson_18.py --verify` to verify automatically.
...
final class _MasteryRubricScreenState
  ...
  static const _all = <_Assertion>[
    ...
    _Assertion(tag:'NAV', label:'BookingStub accessible from detail screen'),
    // STATE
    ...
    _Assertion(tag:'STATE', label:'setSortBy no re-fetch'),
    _Assertion(tag:'STATE', label:'Results screen reads criteria from Provider'),
    _Assertion(tag:'STATE', label:'Sort chips reorder flight list'),
    ...
    _Assertion(tag:'MOCK', label:'Same route+date → same results'),
    ...
    // BONUS
    _Assertion(tag:'BONUS', label:'FlightDetailScreen shows formattedPrice'),
    _Assertion(tag:'BONUS', label:'FlightDetailScreen shows stops label'),
    _Assertion(tag:'BONUS', label:'MasteryRubricScreen is navigable'),
    _Assertion(tag:'BONUS', label:'BookingStub shows flight info'),
  ];
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Mastery Rubric — L18'),
        surfaceTintColor: Colors.transparent,
        leading: BackButton(onPressed: () => context.pop()),
        actions: [
          Semantics(label: done ? 'Uncheck all' : 'Check all', button: true,
            child: TextButton(
            ...
            children: [
              ...
              if (done)
                Row(mainAxisSize: MainAxisSize.min, children: [
                  ExcludeSemantics(child: Icon(Icons.verified,
                    size: 16, color: t.successColor)),
