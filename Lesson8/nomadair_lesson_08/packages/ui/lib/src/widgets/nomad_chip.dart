...
///   FilterChip: [Semantics.selected] reflects toggle state
///   ActionChip: [Semantics.button] marks as one-shot action
///   Touch target: FilterChip/ActionChip enforce 48dp via theme
final class NomadChip extends StatelessWidget {
  const NomadChip({
    super.key,required this.label,required this.onTap,
    this.variant=const FilterChipVariant(),
    this.selected=false,this.icon,this.semanticLabel,
  });
  final String label;
    ...
    return Semantics(
      ...
      child:switch(variant){
        FilterChipVariant()=>FilterChip(
          ...
          avatar:icon!=null
            ?ExcludeSemantics(child:Icon(icon,size:16)):null,
        ...
        ActionChipVariant()=>ActionChip(
          label:Text(label,style:AppTypography.labelMedium.copyWith(color:t.brandPrimary)),
          avatar:icon!=null
            ?ExcludeSemantics(child:Icon(icon,size:16,color:t.brandPrimary)):null,
          onPressed:onTap,
