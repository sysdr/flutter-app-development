...
final class AdaptiveDestination{
  const AdaptiveDestination({required this.icon,required this.selectedIcon,
    required this.label,required this.semanticLabel,required this.builder});
...
final class AdaptiveScaffold extends StatefulWidget{
  const AdaptiveScaffold({super.key,required this.destinations,
    this.title,this.actions,this.initialIndex=0,this.onTabChange})
  ...
  @override State<AdaptiveScaffold> createState()=>_State();
}
...
final class _State extends State<AdaptiveScaffold>{
  ...
  @override
  void initState(){super.initState();_idx=widget.initialIndex;}
  ...
  List<NavigationRailDestination> get _railItems=>widget.destinations
      .map((d)=>NavigationRailDestination(
        icon:Semantics(label:d.semanticLabel,child:Icon(d.icon)),
        selectedIcon:Icon(d.selectedIcon),label:Text(d.label)))
      .toList();
