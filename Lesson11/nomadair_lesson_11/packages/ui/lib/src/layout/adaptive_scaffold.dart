...
final class AdaptiveDestination{
  const AdaptiveDestination({required this.icon,required this.selectedIcon,required this.label,required this.semanticLabel,required this.builder});
  final IconData icon,selectedIcon;final String label,semanticLabel;final WidgetBuilder builder;
}
...
final class _S extends State<AdaptiveScaffold>{
  ...
  @override Widget build(BuildContext c)=>LayoutBuilder(builder:(ctx,cs){
    ...
    // When GoRouter's AdaptiveShellScaffold passes navigationShell as builder result,
    // we show only the first destination's builder — navigationShell handles routing
    final body=widget.destinations[_idx].builder(ctx);
    return AnimatedSwitcher(duration:const Duration(milliseconds:200),
      child:KeyedSubtree(key:ValueKey(style.runtimeType),child:switch(style){
        CompactNav()=>_mob(body),MediumNav()=>_wide(body,false),ExpandedNav()=>_wide(body,true),
      }));
  });
  Widget _mob(Widget body)=>Scaffold(appBar:_bar2(),body:body,
    bottomNavigationBar:NavigationBar(selectedIndex:_idx,onDestinationSelected:_sel,destinations:_bar,height:64));
  Widget _wide(Widget body,bool ext)=>Scaffold(appBar:_bar2(),body:Row(children:[
    NavigationRail(extended:ext,selectedIndex:_idx,onDestinationSelected:_sel,destinations:_rail,useIndicator:true,minWidth:72,minExtendedWidth:180),
    const VerticalDivider(thickness:1,width:1),Expanded(child:body)]));
