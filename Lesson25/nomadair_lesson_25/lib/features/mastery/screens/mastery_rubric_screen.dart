...
final class _MasteryRubricScreenState
  ...
  static const _all = <_Assertion>[
    ...
    _Assertion(tag: 'NAV',   label: 'Tap flight card → FlightDetailScreen'),
    ...
    _Assertion(tag: 'STATE', label: 'Idle → Loading → Loaded state machine'),
    ...
    _Assertion(tag: 'MOCK',  label: '4 non-stop · 4 one-stop · 2 two-stop'),
    _Assertion(tag: 'MOCK',  label: 'Same route+date → same results (seeded RNG)'),
    _Assertion(tag: 'MOCK',  label: 'Business avg > 3× Economy avg price'),
    ...
    _Assertion(tag: 'LINK',  label: 'Unknown IATA → null (graceful)'),
    _Assertion(tag: 'LINK',  label: 'Route accepts from= and to= params'),
    // ── DOMAIN — NEW in L19 (11) ──────────────────────────────────────
    ...
    _Assertion(tag: 'DOMAIN', label: 'formattedDuration: 210 min → 3h 30m'),
    ...
    // ── HTTP — NEW in L21 (9) ─────────────────────────────────────────
    _Assertion(tag: 'HTTP', label: 'http.Client injected — not hardcoded'),
    ...
    _Assertion(tag: 'HTTP', label: '4xx fails immediately — no retry'),
    ...
    _Assertion(tag: 'HTTP', label: 'useMock=false → AmadeusFlightRepository'),
    _Assertion(tag: 'HTTP', label: 'All 28 HTTP tests pass'),
    // ── CACHE — NEW in L22 (7) ───────────────────────────────────────
    _Assertion(tag: 'CACHE', label: 'Cache hit skips network call'),
    _Assertion(tag: 'CACHE', label: 'Expired entry triggers re-fetch'),
    _Assertion(tag: 'CACHE', label: 'TTL matches CachePolicy.flightSearchTtlSeconds'),
    _Assertion(tag: 'CACHE', label: 'Cache key encodes route + date + cabin'),
    _Assertion(tag: 'CACHE', label: 'CacheBadge visible on cached results'),
    _Assertion(tag: 'CACHE', label: 'LRU eviction enforces maxFlightSearchEntries'),
    _Assertion(tag: 'CACHE', label: 'All 20 cache tests pass'),
    // ── DEEPLINK2 — NEW in L23 (7) ───────────────────────────────────
    _Assertion(tag: 'DL2', label: 'from+to+date → SearchIntent (full criteria)'),
    _Assertion(tag: 'DL2', label: 'from+to (no date) → PreFillIntent'),
    _Assertion(tag: 'DL2', label: 'Unknown IATA → InvalidIntent + SnackBar'),
    _Assertion(tag: 'DL2', label: 'SearchScreen accepts initialDate param'),
    _Assertion(tag: 'DL2', label: 'Cache-hit deep link → skip form, go to results'),
    _Assertion(tag: 'DL2', label: "AndroidManifest has host='search'"),
    _Assertion(tag: 'DL2', label: 'All 18 deep link resolver tests pass'),
    // ── ANIM — NEW in L24 (7) ────────────────────────────────────────
    _Assertion(tag: 'ANIM', label: 'Hero tag on FlightResultCard price'),
    _Assertion(tag: 'ANIM', label: 'Hero tag on FlightDetailScreen price'),
    _Assertion(tag: 'ANIM', label: 'Hero tag on DestinationDetailScreen banner'),
    _Assertion(tag: 'ANIM', label: 'HeroTags namespace collision impossible'),
    _Assertion(tag: 'ANIM', label: 'flightDetail uses slideUp CustomTransitionPage'),
    _Assertion(tag: 'ANIM', label: 'discoveryDetail uses slideRight transition'),
    _Assertion(tag: 'ANIM', label: 'All 16 transition tests pass'),
    // ── BOOKING — NEW in L25 (8) ─────────────────────────────────────
    _Assertion(tag: 'BOOK', label: 'confirmBooking: Idle → Loading → Confirmed'),
    _Assertion(tag: 'BOOK', label: '404 → FlightUnavailableError, canRetry=false'),
    _Assertion(tag: 'BOOK', label: '412 / price change → PriceChangedError'),
    _Assertion(tag: 'BOOK', label: 'Network error → canRetry=true, retry() works'),
    _Assertion(tag: 'BOOK', label: '1% price tolerance: within tolerance → Confirmed'),
    _Assertion(tag: 'BOOK', label: 'BookingScreen shows error UI per variant'),
    _Assertion(tag: 'BOOK', label: 'bookingConfirm route uses slideUp transition'),
    _Assertion(tag: 'BOOK', label: 'All 16 booking notifier tests pass'),
  ];
  ...
  Color _tagColor(String tag, NomadThemeExtension t) => switch (tag) {
    ...
    'CACHE'   => const Color(0xFF059669),
    'DL2'     => const Color(0xFF7C3AED),
    'ANIM'    => const Color(0xFF0891B2),
    'BOOK'    => const Color(0xFF166534),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Mastery Rubric — L25'),
