...
final class SharedPrefsThemeService implements ThemePreferenceService {
  ...
  Future<ThemeMode> load() async {
    ...
    final c = (p.getInt(_kL) ?? 0) + 1;
    await p.setInt(_kL, c);
    final i = p.getInt(_kT);
  ...
  Future<int> launchCount() async {
    final p = await SharedPreferences.getInstance();
    return p.getInt(_kL) ?? 0;
  }

  /// Clears singleton for integration tests (same library only).
