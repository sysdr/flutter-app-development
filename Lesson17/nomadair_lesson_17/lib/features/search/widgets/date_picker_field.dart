...
final class DatePickerField extends StatelessWidget {
  const DatePickerField({super.key, required this.label,
    required this.onChanged, this.value, this.firstDate,
    this.errorText, this.enabled = true});
  final String label; final DateTime? value, firstDate;
  final String? errorText; final bool enabled;
  final ValueChanged<DateTime?> onChanged;
  static final _fmt = DateFormat('dd MMM yyyy');
  Future<void> _pick(BuildContext ctx) async {
    final now   = DateTime.now();
    final first = firstDate ?? now;
    final init  = (value != null && value!.isAfter(first)) ? value! : first;
    final p = await showDatePicker(context: ctx,
      initialDate: init, firstDate: first,
  ...
  @override Widget build(BuildContext context) {
    ...
    final d  = value != null ? _fmt.format(value!) : '';
      ...
      InkWell(
        ...
        child: InputDecorator(
          decoration: InputDecoration(
            labelText: label,
            filled: true, fillColor: enabled
              ? t.surfaceColor : t.onSurfaceColor.withAlpha(10),
            prefixIcon: ExcludeSemantics(child: Icon(Icons.calendar_today,
              size: 20, color: t.onSurfaceColor.withAlpha(160))),
            suffixIcon: value != null ? ExcludeSemantics(child:
          ...
          child: Text(d, style: AppTypography.bodyLarge
