...
sealed class ChipVariant {
  const ChipVariant();
}

final class FilterChipVariant extends ChipVariant {
  const FilterChipVariant();
}

final class ActionChipVariant extends ChipVariant {
  const ActionChipVariant();
}
    ...
    return Semantics(
      ...
      child: switch (variant) {
        FilterChipVariant() => FilterChip(
            ...
            side: BorderSide(
              color:
                  selected ? t.brandPrimary : t.onSurfaceColor.withAlpha(60),
            ),
        ...
        ActionChipVariant() => ActionChip(
            label: Text(
              label,
              style:
                  AppTypography.labelMedium.copyWith(color: t.brandPrimary),
            ),
            avatar: icon != null
                ? ExcludeSemantics(
                    child: Icon(icon, size: 16, color: t.brandPrimary),
                  )
                : null,
            onPressed: onTap,
