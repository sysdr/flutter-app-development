...
final class DiscoveryFeedScreen extends StatefulWidget{const DiscoveryFeedScreen({super.key});@override State<DiscoveryFeedScreen> createState()=>_State();}
final class _State extends State<DiscoveryFeedScreen> with SingleTickerProviderStateMixin{
  late final AnimationController _sh;_FS _s=const _Loading();DiscoveryFilter _f=DiscoveryFilter.all;bool _pv=false;
  ...
  List<DiscoveryDestination> get _flt{if(_s is! _Loaded)return[];final all=(_s as _Loaded).items;return switch(_f){DiscoveryFilter.all=>all,DiscoveryFilter.flights=>all.where((d)=>d.category=='flights'||d.category=='both').toList(),DiscoveryFilter.hotels=>all.where((d)=>d.category=='hotels'||d.category=='both').toList()};}
  @override Widget build(BuildContext ctx){final t=Theme.of(ctx).extension<NomadThemeExtension>()!;return Column(crossAxisAlignment:CrossAxisAlignment.start,children:[
    Padding(padding:const EdgeInsets.fromLTRB(AppSpacing.md,AppSpacing.md,AppSpacing.md,0),child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[
      ...
      AnimatedOpacity(opacity:_s is _Loading?0.4:1.0,duration:const Duration(milliseconds:300),child:FilterChipBarWidget(active:_f,onChanged:_s is _Loading?(_){}:(f){setState((){_f=f;if(_s is _Loaded)_s=_Loaded((_s as _Loaded).items,f);});})),
    ])),
    ...
    Expanded(child:AnimatedSwitcher(duration:const Duration(milliseconds:300),child:KeyedSubtree(key:ValueKey('${_s.runtimeType}-$_pv-$_f'),child:switch(_s){
      ...
      _Error()=>Center(child:NomadButton(label:'Retry',icon:Icons.refresh,onPressed:_load)),
    })))]);
  }
