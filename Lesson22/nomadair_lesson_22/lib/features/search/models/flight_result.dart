...
final class FlightResult {
  ...
  factory FlightResult.fromAmadeusJson(Map<String,dynamic> offer){
    ...
    return FlightResult(id:offer['id'] as String??'',airline:_airlineName(airline),airlineCode:airline,flightNumber:'$airline ${first['number'] as String? ?? ''}',origin:(first['departure'] as Map)['iataCode'] as String??'',destination:(last['arrival'] as Map)['iataCode'] as String??'',departureTime:depart,arrivalTime:arrive,durationMinutes:durMin,stops:stops,cabinClass:_cabinFromAmadeus(cabin),priceInr:price,seatsLeft:9,baggageAllowance:'23 kg',isRefundable:false);}
